...
from xiangqi_arena.core.constants import (
    ...
    PALACE_BOUNDS,
    HUMANSIDE_CROSSED_RIVER_X_MAX,
    ORCSIDE_CROSSED_RIVER_X_MIN,
)
...
def is_within_board(x: int, y: int) -> bool:
    """Return True if (x, y) is a legal board node (0..9, 0..8)."""
    return X_MIN <= x <= X_MAX and Y_MIN <= y <= Y_MAX


# ---------------------------------------------------------------------------
# Palace checks
# Rulebook V3 §4.4: Leader/Marshal can only move within its own palace.
# ---------------------------------------------------------------------------
...
def has_crossed_river(x: int, y: int, faction: Faction) -> bool:
    ...
    HumanSide crosses when x <= 4; OrcSide crosses when x >= 5.
    """
    if faction is Faction.HumanSide:
        return x <= HUMANSIDE_CROSSED_RIVER_X_MAX
    else:
        return x >= ORCSIDE_CROSSED_RIVER_X_MIN
...
def neighborhood_3x3(x: int, y: int) -> list[Pos]:
    """
    Return the up-to-8 nodes in the 3×3 area centered on (x, y), excluding
    the centre itself, restricted to nodes within the board.

    Used for Soldier nearby-ally bonus (Rulebook V3 §9.5).
    """
    neighbors = []
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
...
# ---------------------------------------------------------------------------
# Direction helpers (used by Archer / Wizard path building)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Lancer move helpers
# Rulebook V3 §9.3: standard Chinese chess lancer (L-shape with blocking).
...
_LANCER_MOVES: list[tuple[int, int, int, int]] = [
...
def lancer_reachable(x: int, y: int, is_occupied: Callable[[int, int], bool]) -> list[Pos]:
    """
    Return all positions reachable by a Lancer at (x, y).

    *is_occupied* should be a callable(x, y) -> bool that returns True when a
    node is occupied by any live piece (blocking the leg).
    """
    result: list[Pos] = []
    for bx, by, fx, fy in _LANCER_MOVES:
