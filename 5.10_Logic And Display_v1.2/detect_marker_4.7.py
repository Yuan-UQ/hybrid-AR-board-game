...
# 棋盘交叉点：水平 9 个 × 竖直 10 个（内部插值 grid[int_row][int_col] 不变）。
#
# 对外棋盘坐标采用 (board_x, board_y)：
#   board_x ∈ [0, 8]  （列，从左到右）
#   board_y ∈ [0, 9]  （行，从下到上）
#
# 内部插值网格仍是 grid[int_row][int_col]（int_row 从上到下，int_col 从左到右）。
# 本项目将对外坐标仅做“Y 轴翻转”，使四角满足：
#   (0,9)→(0,0)；(0,0)→(0,9)；(8,9)→(8,0)；(8,0)→(8,9)
# 即：board_x = int_col, board_y = 9 - int_row
BOARD_COLS = 9   # 内部 grid 第二维 int_col = 0~8
BOARD_ROWS = 10  # 内部 grid 第一维 int_row = 0~9


def board_cell_from_internal(int_col: int, int_row: int) -> tuple[int, int]:
    """内部 grid 下标 → 对外棋盘坐标 (board_x, board_y)。"""
    return (int_col, 9 - int_row)


def internal_indices_from_board(board_x: int, board_y: int) -> tuple[int, int]:
    """对外棋盘坐标 → grid[int_row][int_col] 的内部下标。"""
    return (9 - board_y, board_x)
...
# 接入游戏时：red_*（10–16）应对应「实体红棋」→ HumanSide；black_*（17–23）→ 实体黑棋 → OrcSide
# （映射表见 xiangqi_arena/recognition/aruco_tracker.py 的 MARKER_LABEL_TO_PIECE_ID）
PIECE_ARUCO_IDS = {
    10: "red_general",
    11: "red_chariot",
    12: "red_horse",
...
def get_board_points_from_semantic_corners(detected_markers, corner_offsets):
    """
    按四角 marker 固定透视；交叉点对外坐标为 (board_x, board_y)，board_x∈[0,8], board_y∈[0,9]。

    几何四角语义（与 BOARD_MARKER_IDS 命名对应）：
    TL = BLACK_LEFT
    TR = BLACK_RIGHT
    BR = RED_RIGHT
    BL = RED_LEFT

    marker 贴在棋盘外侧安全位置，marker 参考点 + offset = 棋盘真实角点
    """
    required_ids = [
...
def compute_board_grid(board_corners):
    """
    根据四角点，生成 9×10 交叉点的像素坐标（内部索引 grid[int_row][int_col]）。

    返回:
        grid[int_row][int_col] = (px, py)；对外坐标用 board_cell_from_internal(int_col, int_row)
    """
...
def generate_grid_points_warp(warp_w, warp_h, margin=0):
    """鸟瞰图中的 10×9 内部均匀交叉点（与 compute_board_grid 拓扑一致）。"""
    points = []
...
def nearest_board_cell(px, py, grid, radius_mult=0.55):
    """将图像坐标映射到最近交叉点 (board_x, board_y)；过远返回 None。radius_mult 为相对格宽的倍数。"""
    if grid is None:
        return None
    best = None
    best_d = 1e18
    for int_row in range(BOARD_ROWS):
        for int_col in range(BOARD_COLS):
            gx, gy = grid[int_row][int_col]
            d = (float(gx) - px) ** 2 + (float(gy) - py) ** 2
            if d < best_d:
                best_d = d
                best = (int_col, int_row)
    ...
    if best_d > thresh:
        return None
    int_col, int_row = best
    return board_cell_from_internal(int_col, int_row)
...
def draw_piece_labels(
    ...
    for i, mid in enumerate(ids.flatten()):
        ...
        name = PIECE_ARUCO_IDS[mid]
        if swap_piece_side_labels:
            if name.startswith("red_"):
                name = "black_" + name[len("red_"):]
            elif name.startswith("black_"):
                name = "red_" + name[len("black_"):]
        cell = nearest_board_cell(fx, fy, grid, cell_radius_mult)
...
def draw_polyline_by_board_coords(frame, grid, coords, color, thickness=2):
    """
    coords: [(board_x, board_y), ...]，与 nearest_board_cell 返回值一致。
    """
    pts = np.array(
        [
            grid[ir][ic]
            for bx, by in coords
            for (ir, ic) in (internal_indices_from_board(bx, by),)
        ],
        dtype=np.int32,
    )
...
def draw_board_overlay(frame, grid, swap_side_labels=False):
    # 画所有横线
    for y in range(BOARD_ROWS):
    ...
    # 画交叉点 + 对外棋盘坐标 (board_x, board_y)
    for int_row in range(BOARD_ROWS):
        for int_col in range(BOARD_COLS):
            px, py = grid[int_row][int_col]
            bx, by = board_cell_from_internal(int_col, int_row)
    ...
    red_area_color = (255, 0, 0) if swap_side_labels else (0, 0, 255)
    black_area_color = (0, 0, 255) if swap_side_labels else (255, 0, 0)

    # 红方九宫格（对外坐标）：x 3~5, y 0~2
    draw_closed_polygon_by_board_coords(
        frame, grid,
        [(3, 0), (5, 0), (5, 2), (3, 2)],
        color=red_area_color,
        thickness=2
    )
    draw_polyline_by_board_coords(frame, grid, [(3, 0), (5, 2)], color=red_area_color, thickness=2)
    draw_polyline_by_board_coords(frame, grid, [(5, 0), (3, 2)], color=red_area_color, thickness=2)

    # 黑方九宫格（对外坐标）：x 3~5, y 7~9
    draw_closed_polygon_by_board_coords(
        frame, grid,
        [(3, 7), (5, 7), (5, 9), (3, 9)],
        color=black_area_color,
        thickness=2
    )
    draw_polyline_by_board_coords(frame, grid, [(3, 7), (5, 9)], color=black_area_color, thickness=2)
    draw_polyline_by_board_coords(frame, grid, [(5, 7), (3, 9)], color=black_area_color, thickness=2)

    # 简单标签（按对外坐标取点）
    r_ir, r_ic = internal_indices_from_board(4, 0)   # 红方上沿中点
    b_ir, b_ic = internal_indices_from_board(4, 9)   # 黑方下沿中点
    red_label_pos = grid[r_ir][r_ic]
    black_label_pos = grid[b_ir][b_ic]

    red_side_label = "BLACK SIDE" if swap_side_labels else "RED SIDE"
    black_side_label = "RED SIDE" if swap_side_labels else "BLACK SIDE"
    red_side_color = (255, 0, 0) if swap_side_labels else (0, 0, 255)
    black_side_color = (0, 0, 255) if swap_side_labels else (255, 0, 0)

    cv2.putText(frame, red_side_label, (red_label_pos[0], red_label_pos[1] - 15),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, red_side_color, 2)

    cv2.putText(frame, black_side_label, (black_label_pos[0], black_label_pos[1] + 25),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, black_side_color, 2)
...
def draw_status_panel(
    ...
    put("Board: 9x10 (board_x 0-8 L->R, board_y 0-9 B->T)", (180, 180, 180), 0.55, 25)
    put("River: internal rows 4/5", (180, 180, 180), 0.55, 25)
    put("Red palace: bx 3-5 by 0-2", (180, 180, 180), 0.55, 25)
    put("Black palace: bx 3-5 by 7-9", (180, 180, 180), 0.55, 25)
...
def main():
    ...
    board_points = None
    last_valid_board_points = None
    board_points_missing_streak = 0
    board_points_cache_grace_frames = 100
    ...
    while True:
        ...
        observed_board_points = get_board_points_from_semantic_corners(
            detected_markers, _offsets_as_tuples(corner_state)
        )
        board_points_from_cache = False
        if observed_board_points is not None:
            board_points = dict(observed_board_points)
            last_valid_board_points = dict(observed_board_points)
            board_points_missing_streak = 0
        else:
            board_points_missing_streak += 1
            if (
                last_valid_board_points is not None
                and board_points_missing_streak <= board_points_cache_grace_frames
            ):
                board_points = dict(last_valid_board_points)
                board_points_from_cache = True
            else:
                board_points = None
