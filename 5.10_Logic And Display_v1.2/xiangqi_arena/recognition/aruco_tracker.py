...
# Display-only debug preview orientation. Detection data and game coordinates
# stay in the original camera coordinate system.
PREVIEW_ROTATE_180: bool = True

# Display-only side correction for physical camera/board orientation. This
# swaps debug overlay text only; game coordinates and rules stay unchanged.
SWAP_RED_BLACK_SIDE: bool = True
...
class VisionTracker:
    ...
    def read_positions(self, exclude_piece_ids: frozenset[str] | None = None) -> VisionFrame:
        """Return a ``VisionFrame`` of stable board cells.

        *exclude_piece_ids* should list logical piece IDs that are already dead
        in game state but may still linger in ``piece_last_cell`` after the
        physical marker was removed. Omitting them avoids overlap-pruning
        dropping a living piece that moved onto the victim's cell (otherwise
        ``positions`` can end up empty for both pieces and the UI hides the
        living sprite when using ``positions.keys()`` as ``visible_piece_ids``).
        """
        ...
        ex = exclude_piece_ids or frozenset()
        positions, conflicts = self._stable_cells_to_game_positions(ex)
        if positions:
            self.last_positions = positions

        raw_map = self._raw_piece_cells_to_game_positions(ex)
    ...
    def _stable_cells_to_game_positions(
        self,
        exclude_piece_ids: frozenset[str],
        ...
        for marker_id, cell in self.piece_last_cell.items():
            ...
            if piece_id is None:
                continue
            if piece_id in exclude_piece_ids:
                continue
    ...
    def _raw_piece_cells_to_game_positions(
        self,
        exclude_piece_ids: frozenset[str],
    ...
    def _draw_debug(
        ...
        display_select_centers = select_centers

        if PREVIEW_ROTATE_180:
            display_frame = self.cv2.rotate(display_frame, self.cv2.ROTATE_180)
            h, w = frame.shape[:2]
            display_grid = self._rotate_grid_180(grid, w, h) if grid is not None else None
            display_corners = (
                self._rotate_corners_180(corners, w, h)
                if corners is not None
                else None
            )
            display_select_centers = (
                [self._rotate_point_180(cx, cy, w, h) for cx, cy in select_centers]
                if select_centers
                else select_centers
            )

        if ids is not None and len(ids) > 0 and display_corners is not None:
            self.cv2.aruco.drawDetectedMarkers(display_frame, display_corners, ids)
        if board_ok and display_grid is not None:
            self.dm.draw_board_overlay(
                display_frame,
                display_grid,
                swap_side_labels=SWAP_RED_BLACK_SIDE,
            )
            self.dm.draw_piece_labels(
                ...
                self.piece_off_side,
                self.piece_cell_mult,
                swap_piece_side_labels=SWAP_RED_BLACK_SIDE,
            )

        if display_select_centers:
            for cx, cy in display_select_centers:
                self.cv2.circle(display_frame, (int(cx), int(cy)), 18, (0, 255, 255), 3)
                self.cv2.putText(
                    display_frame,
                    "SELECT",
                    (int(cx) + 22, int(cy) - 10),
        ...
        self.cv2.imshow("Xiangqi Arena Vision", display_frame)
        self.cv2.waitKey(1)

    @staticmethod
    def _rotate_point_180(x: float, y: float, width: int, height: int) -> tuple[int, int]:
        return int(width - 1 - x), int(height - 1 - y)

    def _rotate_grid_180(self, grid: Any, width: int, height: int) -> list[list[tuple[int, int]]]:
        return [
            [self._rotate_point_180(px, py, width, height) for px, py in row]
            for row in grid
        ]

    def _rotate_corners_180(self, corners: Any, width: int, height: int) -> Any:
        rotated = []
        for marker_corners in corners:
            pts = marker_corners.copy()
            pts[..., 0] = width - 1 - pts[..., 0]
