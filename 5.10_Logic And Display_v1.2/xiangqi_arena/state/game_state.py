...
from xiangqi_arena.core.constants import (
    ...
    HUMANSIDE_ARCHER_START, ORCSIDE_ARCHER_START,
...
def build_initial_state(
    ...
    HumanSide_free:
        ...
        Lancer/Wizard must be on x = HUMANSIDE_DEPLOY_X (9); Archer uses
        HUMANSIDE_ARCHER_START to avoid covering board-corner markers.
    OrcSide_free:
        Same for OrcSide; Archer uses ORCSIDE_ARCHER_START.
    ...
    for pt in FREE_DEPLOY_PIECE_TYPES:
        ...
        human_pos = tuple(HumanSide_free[pt])
        orc_pos = tuple(OrcSide_free[pt])
        rx, _ = human_pos
        if pt is PieceType.ARCHER:
            if human_pos != HUMANSIDE_ARCHER_START:
                raise ValueError(
                    f"HumanSide {pt} must start at {HUMANSIDE_ARCHER_START}, got {human_pos}"
                )
        elif rx != HUMANSIDE_DEPLOY_X:
            raise ValueError(f"HumanSide {pt} must be on x={HUMANSIDE_DEPLOY_X}, got x={rx}")
        bx, _ = orc_pos
        if pt is PieceType.ARCHER:
            if orc_pos != ORCSIDE_ARCHER_START:
                raise ValueError(
                    f"OrcSide {pt} must start at {ORCSIDE_ARCHER_START}, got {orc_pos}"
                )
        elif bx != ORCSIDE_DEPLOY_X:
...
def build_default_state() -> GameState:
    ...
    Uses a symmetric default deployment:
      OrcSide: Archer(4,8), Lancer(0,6), Wizard(0,2)
      HumanSide: Archer(5,0), Lancer(9,2), Wizard(9,6)
    """
    return build_initial_state(
        HumanSide_free={
            PieceType.ARCHER: HUMANSIDE_ARCHER_START,
            PieceType.LANCER:  (9, 2),
            PieceType.WIZARD: (9, 6),
        },
        OrcSide_free={
            PieceType.ARCHER: ORCSIDE_ARCHER_START,
