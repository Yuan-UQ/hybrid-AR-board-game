...
Two public functions:
  apply_attack(attacker_id, target_pos, state)
      Single-target attack — used by all pieces except the Wizard.

  apply_wizard_attack(attacker_id, center_pos, state)
      Cross-shaped AOE attack — used by the Wizard only.
...
from xiangqi_arena.rules.buff_rules import get_soldier_attack_effect_bonus
from xiangqi_arena.rules.attack_rules import get_wizard_aoe
...
def apply_wizard_attack(
    attacker_id: str,
    center_pos: Pos,
    state: GameState,
) -> None:
    """
    Apply the Wizard's cross-shaped AOE attack.
    ...
    """
    attacker: Piece = state.pieces[attacker_id]
    aoe_nodes = get_wizard_aoe(center_pos, state, attacker.faction)
...
def _refresh_victory(state: GameState) -> None:
    ...
    game ends *immediately* when the Leader/Marshal reaches HP ≤ 0).
    """
...
def _record_attack(
    ...
    attacker_has_attack_buff = get_soldier_attack_effect_bonus(attacker, state) > 0
    state.history.append({
        "type": "attack",
        "round": state.round_number,
        "attacker_id": attacker.id,
        "attacker_has_attack_buff": attacker_has_attack_buff,
        "target_id": target.id,
