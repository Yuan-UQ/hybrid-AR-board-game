...
Rulebook V3 §15:
  §15.1 Victory: if either General/Marshal reaches HP ≤ 0, the opposing
...
def check_victory(state: GameState) -> VictoryState:
    ...
    Priority order:
    1. General/Marshal HP ≤ 0   →  immediate win for the opponent.
    ...
    """
    # 1. General/Marshal death (Rulebook V3 §15.1)
    for faction in (Faction.RED, Faction.BLACK):
        general = state.general_of(faction)
        if general.hp <= 0:
            return (
                VictoryState.BLACK_WIN
                if faction is Faction.RED
                else VictoryState.RED_WIN
            )
