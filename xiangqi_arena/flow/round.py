...
Rulebook V3 §10.4 definition:
  1 round = Red completes 1 turn + Black completes 1 turn.

The round counter starts at 1 and increments *after* Black finishes each
turn (i.e. just before Red's next START phase begins).

Event point spawn timing (corrected rule):
  One event point is spawned at the START of Red's turn on odd-numbered
  ...
  Key: spawning is checked only when it is Red's START phase so that Black's
...
def increment_round(state: GameState) -> None:
    ...
    Called by `flow.turn.end_turn()` immediately after Black's RESOLVE phase,
...
def should_spawn_event_point(state: GameState) -> bool:
    ...
    2. It is the beginning of Red's turn — Red always opens a new round, so
       this gate ensures we spawn exactly once per odd round, not twice
       (once for Red, once for Black in the same round).
    """
    return (
        is_event_point_round(state.round_number)
        and state.active_faction is Faction.RED
    )
