    ...
    * Otherwise (selected piece reappears at NEW cell, no SELECT visible):
    ...
    if pending_attack_commit_ref[0] is not None:
        ...
        if commit.get("pid") != pid:
            ...
            pending_attack_commit_ref[0] = None
            action_streak_ref[0] = {}
            _push_retract(pending_retracts, pid, piece.faction, visible_pos, from_pos)
            return [
    ...
    if (
        ...
        _push_retract(pending_retracts, pid, piece.faction, new_pos, from_pos)
        return [
...
def main() -> None:
    ...
    while running:
        ...
        can_action_buttons = (not game_over) and state.current_phase is Phase.MOVEMENT
        ...
        if vision_tracker is not None and not game_over:
            ...
            if now_ms >= next_vision_sync_ms:
                ...
                vision_messages = _sync_state_to_vision(
                    state, vision_status.positions, sel, pending_retracts
                )
        ...
        if pending_retracts:
            now_anim_ms = pygame.time.get_ticks()
            pending_retracts[:] = [r for r in pending_retracts if not r.is_finished(now_anim_ms)]
