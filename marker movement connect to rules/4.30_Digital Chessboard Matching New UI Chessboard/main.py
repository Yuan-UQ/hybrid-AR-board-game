...
def _sync_state_to_vision(
    ...
    new_occupancy = {
        pos: pid
        for pos, pid in current_occupancy.items()
        if pid not in valid_updates
    }
    changed: list[str] = []
    for pid, pos in valid_updates.items():
        piece = state.pieces[pid]
        if tuple(piece.pos) != tuple(pos):
            changed.append(f"{pid} -> {pos}")
        piece.pos = pos
        previous_owner = new_occupancy.pop(pos, None)
        if previous_owner is not None and previous_owner != pid:
            messages.append(f"Vision overrides preset {previous_owner} at {pos}")
        new_occupancy[pos] = pid

    state.board._occupancy = new_occupancy
    ...
    if sel.has_selection and sel.selected_pid in state.pieces:
        ...
        if selected.is_dead:
            ...
            sel.select(selected.id, selected.pos, moves=legal_moves(selected, state), attacks=[])
    ...
    if args.vision:
        try:
            ...
            for _ in range(max(0, int(args.vision_warmup_frames))):
                ...
                if vision_status.positions:
                    _sync_state_to_vision(state, vision_status.positions, sel)
                if (
    ...
    while running:
        ...
        if vision_tracker is not None and not game_over:
            ...
            if now_ms >= next_vision_sync_ms:
                ...
                vision_messages = _sync_state_to_vision(state, vision_status.positions, sel)
        ...
        if args.vision and vision_status is not None:
            visible_piece_ids = set(vision_status.positions.keys())
        draw_pieces(screen, state, visible_piece_ids=visible_piece_ids)
