...
def _update_sprite_facing(piece_id: str, board_x: int) -> str:
    if piece_id not in _SPRITE_FACING:
        _SPRITE_FACING[piece_id] = str(_SPRITE_CONFIG[piece_id].get("initial_facing", "right"))

    previous_x = _SPRITE_LAST_X.get(piece_id)
    if previous_x is not None:
        if board_x < previous_x:
            _SPRITE_FACING[piece_id] = "left"
        elif board_x > previous_x:
            _SPRITE_FACING[piece_id] = "right"

    _SPRITE_LAST_X[piece_id] = board_x
...
def _active_sprite_animation(piece_id: str) -> dict | None:
...
def _draw_piece_sprite(
    ...
    if _update_sprite_facing(piece_id, board_x) == "left":
...
def draw_pieces(
    ...
    *,
    ...
    for piece in state.pieces.values():
        ...
        animation = _active_sprite_animation(piece.id)
        if (
            piece.is_dead
        ...
        if _draw_piece_sprite(screen, piece.id, piece.pos[0], px, py, animation):
