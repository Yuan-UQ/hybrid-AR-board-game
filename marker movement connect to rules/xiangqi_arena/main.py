...
def _handle_vision_movement(
    ...
    if piece is None:
        return [f"Vision move ignored: unknown piece {event.piece_id}."]
    if piece.faction is not state.active_faction:
        return [f"Vision move ignored: {event.piece_id} is not the active side."]
    if not can_select_piece(state, event.piece_id):
        return [f"Vision move ignored: {event.piece_id} cannot be selected now."]
    if piece.pos != event.from_pos:
        if piece.pos == event.to_pos:
            return []
        return [
            f"Vision move ignored: {event.piece_id} backend at {piece.pos}, "
    ...
    try:
        while running:
            ...
            if not game_over:
                ...
                if phase == Phase.MOVEMENT:
                    ...
                    for event in vision_events:
                        for message in _handle_vision_movement(state, sel, event):
                            _push(log, message)
                        if state.current_phase is not Phase.MOVEMENT:
                            break
            ...
            draw_panel(
                ...
                btn_label=btn_lbl,
                btn_hover=btn_hover,
                instruction_override=_vision_instruction(scanner is not None, state.current_phase),
            )
