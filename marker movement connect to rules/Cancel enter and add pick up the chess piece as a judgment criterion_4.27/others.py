...
_PHASE_INSTRUCTIONS: dict[Phase, str] = {
    ...
    Phase.MOVEMENT:    "Select one piece, then move or attack once",
    Phase.RECOGNITION: "",   # auto-processed
    Phase.ATTACK:      "Attack resolves automatically",
...
def draw_panel(
    ...
    # --- Instructions ---
    instr = instruction_override if instruction_override is not None else _PHASE_INSTRUCTIONS.get(state.current_phase, "")
    ...
    if status_lines:
        ...
        for raw in status_lines:
            for line in _wrap(raw, fb, PANEL_W - PANEL_PAD * 2):
                s = fb.render(line, True, C_TRAP)
                screen.blit(s, (PANEL_X + PANEL_PAD, y))
                y += s.get_height() + 2
        y += 6
