...
def legal_attack_targets(piece: Piece, state: GameState) -> list[Pos]:
    ...
    For non-Cannon pieces: a list of enemy-occupied positions within reach.
    For the Cannon: a list of valid *center* positions (one per valid attack
    direction — the player then picks one via arrow keys).
    """
    return get_legal_attack_targets(piece, state)


def cannon_aoe_nodes(
    piece: Piece,
    center_pos: Pos,
    state: GameState,
) -> list[Pos]:
    """
    Return the nodes affected by a Cannon attack aimed at *center_pos*.
    ...
    """
    return get_cannon_aoe(center_pos, state, piece.faction)
