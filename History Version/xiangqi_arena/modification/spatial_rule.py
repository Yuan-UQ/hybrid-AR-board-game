...
1. General / Marshal palace damage reduction (§9.1, §11.2)
...
2. Pawn nearby-ally attack bonus (§9.5)
...
mutations.  This module therefore provides:
  ...
  - `get_pawn_bonus(piece, state)` — convenience accessor for UI / debug use.
...
def get_palace_reduction(piece: Piece) -> int:
    ...
    Only applies to the General / Marshal inside its own palace (+1 reduction).
    Returns 0 for all other pieces or when the General is outside its palace.
    """
    if piece.piece_type is not PieceType.GENERAL:
        return 0
    return 1 if is_in_palace(*piece.pos, piece.faction) else 0


def get_pawn_bonus(pawn: Piece, state: GameState) -> int:
    """
    Return the temporary attack bonus for *pawn* before it attacks.

    Returns 1 if any live friendly piece is within pawn's 3×3 neighbourhood,
    0 otherwise.  Only meaningful when pawn.piece_type is PieceType.PAWN.
    """
    if pawn.piece_type is not PieceType.PAWN:
        return 0
    for pos in neighborhood_3x3(*pawn.pos):
...
def describe_spatial_context(piece: Piece, state: GameState) -> list[str]:
    ...
    Examples: ["Palace: -1 incoming damage", "Pawn bonus: +1 ATK this attack"]
    """
    notes: list[str] = []

    if piece.piece_type is PieceType.GENERAL and get_palace_reduction(piece):
        notes.append("In palace: −1 incoming damage")

    if piece.piece_type is PieceType.PAWN and get_pawn_bonus(piece, state):
