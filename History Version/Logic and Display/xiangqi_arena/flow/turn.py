...
A turn belongs to one player (HumanSide or OrcSide).  At most one friendly piece may
...
3. `end_turn(state)`     — called when the player confirms at RESOLVE.
                            If OrcSide just finished, increments the round first.
...
def end_turn(state: GameState) -> None:
    ...
    Order of operations:
    1. If OrcSide just finished, increment the round counter (HumanSide + OrcSide = 1
    ...
    # Round increments after OrcSide completes a turn (imported lazily to avoid
    # a potential circular reference at module load time).
    if state.active_faction is Faction.OrcSide:
